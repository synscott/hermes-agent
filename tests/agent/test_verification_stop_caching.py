...
As of #65919 §7, the assistant response is NO LONGER flagged synthetic — it is
real content that persists and is emitted to the UI as an interim message.
Only the nudge (the synthetic user message) is flagged, so only the nudge gets
stripped from the durable transcript. This test file verifies:

  - The verification-loop flags remain registered in
    ``_EPHEMERAL_SCAFFOLDING_FLAGS`` (so nudges are still stripped).
...
def test_verification_flags_registered_as_ephemeral(tmp_path, monkeypatch):
    ...
    # Real messages (including the now-unflagged assistant candidate) are not.
...
def test_db_flush_drops_only_nudge_keeps_candidate(tmp_path, monkeypatch):
    ...
    # The assistant candidate ("premature done") now persists — it is real content.
...
def test_json_log_drops_only_nudge_keeps_candidate(tmp_path, monkeypatch):
    ...
    # The assistant candidate ("premature done") now persists.
